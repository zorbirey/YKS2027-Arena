...
if(!Number(programPrefs.questionTarget)||Number(programPrefs.questionTarget)<1)programPrefs.questionTarget=BENCHMARK_QUESTIONS;
...
function balancedProgram(){const target=Math.round(Number(programPrefs.questionTarget)||BENCHMARK_QUESTIONS);const plan=WEEK_PROGRAM.map(function(item){return {day:item.day,topics:item.topics.slice(),total:target,rest:false};});if(!programPrefs.wantsRest||!programPrefs.restDay)return plan;const restIndex=plan.findIndex(function(item){return item.day===programPrefs.restDay;});if(restIndex<0)return plan;plan[restIndex].total=0;plan[restIndex].rest=true;const active=plan.filter(function(_,index){return index!==restIndex;});const extraBase=Math.floor(target/active.length);const extraRemainder=target%active.length;active.forEach(function(item,index){item.total+=extraBase+(index<extraRemainder?1:0);});return plan;}
function syncTargetInputs(){document.getElementById('questionTarget').value=programPrefs.questionTarget;document.getElementById('settingsQuestionTarget').value=programPrefs.questionTarget;}
function commitQuestionTarget(target,source){programPrefs.questionTarget=target;saveProgramPrefs();syncTargetInputs();document.getElementById('targetConfirmModal').classList.remove('show');document.getElementById('targetSummary').textContent='Günlük hedefin '+target+' soru. Bu bir üst sınır değil; daha fazlasını çözebilirsin.';document.getElementById('settingsTargetStatus').textContent='Günlük hedef '+target+' soru olarak güncellendi.';pendingTarget=null;renderProgram();if(source==='settings')document.getElementById('settingsQuestionTarget').focus();}
function requestQuestionTarget(value,source){const target=Math.round(Number(value));const feedback=source==='settings'?document.getElementById('settingsTargetStatus'):document.getElementById('targetSummary');if(!target||target<1){feedback.textContent='En az 1 soruluk geçerli bir hedef gir.';return;}if(target<BENCHMARK_QUESTIONS){pendingTarget=target;pendingTargetSource=source;document.getElementById('targetConfirmText').textContent='Günlük '+target+' soru, '+BENCHMARK_QUESTIONS+' soruluk başlangıç referansımızın altında. Çalışma programınız seçtiğiniz şartlara göre oluşturuluyor. Emin misiniz?';document.getElementById('targetConfirmModal').classList.add('show');return;}commitQuestionTarget(target,source);}
function renderProgram(){const list=document.getElementById('programList');const status=document.getElementById('programStatus');const wrap=document.getElementById('restDayWrap');const select=document.getElementById('restDay');if(!list)return;syncTargetInputs();document.getElementById('targetSummary').textContent='Günlük hedefin '+programPrefs.questionTarget+' soru • Haftalık ölçüm '+(programPrefs.questionTarget*7)+' soru • Üst sınır yok.';wrap.hidden=programPrefs.wantsRest!==true;select.value=programPrefs.restDay||'';document.getElementById('restYes').classList.toggle('selected-choice',programPrefs.wantsRest===true);document.getElementById('restNo').classList.toggle('selected-choice',programPrefs.wantsRest===false);if(programPrefs.wantsRest&&programPrefs.restDay){status.textContent=programPrefs.restDay+' dinlenme günü. O günün '+programPrefs.questionTarget+' soruluk hedefi haftanın diğer günlerine dengeli dağıtıldı.';}else if(programPrefs.wantsRest){status.textContent='Dinlenmek istediğin günü seç ve planı dengele.';}else{status.textContent='Program yedi güne dengeli olarak devam ediyor.';}list.innerHTML=balancedProgram().map(function(item,index){const counts=taskCounts(item.total);return '<article class="schedule-day'+(item.rest?' rest-day':'')+'"><div class="schedule-head"><span>'+(index+1)+'</span><div><h3>'+item.day+'</h3><small>'+(item.rest?'Yenilenme günü':item.total+' soru hedefi • en az '+BENCHMARK_HOURS+' net saat')+'</small></div></div>'+(item.rest?'<div class="rest-message">Bugün dinlen, enerjini yenile. Haftalık soru hedefin diğer günlere dağıtıldı.</div>':'<ul>'+item.topics.map(function(topic,topicIndex){return '<li>'+topic+' • <b>'+counts[topicIndex]+' soru</b></li>';}).join('')+'</ul>')+'</article>';}).join('');renderTodayFocus();}
...
settingsBtn.addEventListener('click',function(){detectPlatform();syncTargetInputs();document.getElementById('settingsTargetStatus').textContent='Mevcut günlük hedef: '+programPrefs.questionTarget+' soru.';settingsModal.classList.add('show');});settingsClose.addEventListener('click',function(){settingsModal.classList.remove('show');});settingsModal.addEventListener('click',function(e){if(e.target===settingsModal)settingsModal.classList.remove('show');});
...
if('serviceWorker' in navigator){window.addEventListener('load',function(){navigator.serviceWorker.register('./sw.js?v=20260823-03').catch(function(){});});}
renderReports();
