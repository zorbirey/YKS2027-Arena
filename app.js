...
function isPremium(){if(window.YksArenaPlans)return window.YksArenaPlans.atLeast('premium');const membership=window.YKS2027_MEMBERSHIP||safeStoredJson(MEMBERSHIP_KEY,{});return ['premium','pro','pro_plus'].includes(membership.plan)&&membership.verified===true;}
...
function loadPlacementData(){if(!placementDataPromise){placementDataPromise=fetch('./data/placements-2025.json?v=20260824-09').then(function(response){if(!response.ok)throw new Error('Yerleştirme verisi yüklenemedi.');return response.json();});}return placementDataPromise;}
